...
export function StatsCards() {
  ...
  return (
    <div className="space-y-6">
      {/* Main Stats */}
      <div className="webview-grid grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {statsData.map((stat, index) => (
          <Card key={index} className="webview-optimized">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium webview-text">{stat.title}</CardTitle>
              <stat.icon className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold webview-text">{stat.value}</div>
              <div className="flex items-center text-xs text-muted-foreground webview-text">
                {stat.trend === "up" ? (
